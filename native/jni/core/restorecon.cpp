...
#define JOKER_CON  "u:object_r:" SEPOL_FILE_TYPE ":s0"
...
void restore_tmpcon() {
    ...
    if (JOKERTMP == "/sbin")
        setfilecon(JOKERTMP.data(), ROOT_CON);
    else
        chmod(JOKERTMP.data(), 0700);

    auto dir = xopen_dir(JOKERTMP.data());
